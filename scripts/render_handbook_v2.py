...
def render_html(profile, places, out):
    ...
    (out/"index.html").write_text("<!doctype html><html lang='zh-CN'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>"+esc(title)+"旅行手册</title><style>"+css+extras+directory_overrides+place_overrides+directory_compact+directory_single_column+preparation_styles+food_styles+experience_styles+shopping_styles+notes_styles+language_styles+adjust_styles+adjust_modal_styles+editorial_styles+"</style></head><body><main>"+body+"</main>"+adjust_html+checklist_script+adjust_script+adjust_modal_script+structured_adjust_script+"</body></html>", encoding="utf-8")
